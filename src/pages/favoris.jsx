...
function Favoris() {
  ...
  const remove = (e) => {
    const cardRemoved = compteActuel?.favorites.find(fav => fav.favdata.nom === e.target.getAttribute("id"))
    const updatedAccount = { ...compteActuel, favorites: [...compteActuel.favorites, cardRemoved] };
                ...
                <div className="flex flex-col gap-4 mt-3">
                  ...
                  <div className="flex justify-between items-center">
                    <Link href={fav.favdata.url}>
                      <Button color="dark">Vers {fav.favdata.nom.replace(/[0-9,()]/g, '')}</Button>
                    </Link>
                    <Button color="dark" onClick={remove} id={fav.favdata.nom}>
                      <GoTrashcan />
                    </Button>
                  </div>
                </div>
